...
def test_webhook(webhook_id: int, _: AuthContext = Depends(require_menu("integrations")), db: Session = Depends(get_db)):
    """Validate that a webhook config exists; sending is left for a later notification worker."""
    item = db.get(IntegrationWebhook, webhook_id)
    if item is None:
        raise HTTPException(status_code=404, detail="Webhook not found")
    return {"status": "ok", "message": "Webhook config is valid and ready for notification worker integration"}
