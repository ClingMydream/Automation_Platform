...
const ACTIONS = [
  ...
  ['press', '键盘操作'], ['wait', '等待'], ['detect_visible', '检测元素是否出现'], ['validate_onboarding_page', '确认引导页并继续'], ['assert_visible', '断言元素可见'], ['assert_hidden', '断言元素已隐藏'], ['assert_in_viewport', '检查元素完整显示'],
...
export function UiAutomationPage({ client, onClose, embedded = false }) {
  ...
  const copyDiagnostic = async (value, label = '内容') => {
    try {
      if (navigator.clipboard && window.isSecureContext) await navigator.clipboard.writeText(value);
      else {
        const textArea = document.createElement('textarea'); textArea.value = value;
        textArea.style.cssText = 'position:fixed;left:-9999px;top:0;opacity:0'; document.body.appendChild(textArea);
        try { textArea.focus(); textArea.select(); if (!document.execCommand('copy')) throw new Error('浏览器拒绝复制'); }
        finally { document.body.removeChild(textArea); }
      }
      message.success(`已复制${label}`);
    } catch (_) { message.error(`复制${label}失败，请手动复制下方内容`); }
  };
