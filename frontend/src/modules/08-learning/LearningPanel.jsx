...
export function LearningPanel({client}) {
  ...
  const [studyTimer,setStudyTimer]=useState({status:'not_started',elapsed_seconds:0});
  const reviewDraftDay=useRef(null);
  const [planOrder,setPlanOrder]=useState('desc');
  ...
  async function load(preferredReviewDay){await client.post(`${API}/schedule/reconcile`,{}); const [o,t,f,p]=await Promise.all([client.get(`${API}/overview`),client.get(`${API}/tasks`),client.get(`${API}/note-folders`),client.get(`${API}/plans`)]); setOverview(o);setTasks(t);setFolders(f);setPlans(p); const day=preferredReviewDay||o.next_task?.day_number;if(reviewDraftDay.current===day)return;if(o.latest_checkin?.learning_day===day)setCheckin(o.latest_checkin);else setCheckin({actual_minutes:0,gains:'',blockers:'',tomorrow_focus:''});reviewDraftDay.current=day;}
  ...
  useEffect(()=>{const day=overview?.next_task?.day_number;if(!day)return;client.post(`${API}/timer/${day}/start?automatic=true`,{}).then(setStudyTimer).catch(error=>message.error(error.message))},[overview?.next_task?.day_number]);
  useEffect(()=>{if(studyTimer.status!=='running')return;const interval=setInterval(()=>setStudyTimer(value=>({...value,elapsed_seconds:value.elapsed_seconds+1})),1000);return()=>clearInterval(interval)},[studyTimer.status]);
  async function toggle(task,checked){const dayTasks=tasks.filter(item=>item.day_number===task.day_number);if(checked&&dayTasks.every(item=>item.id===task.id||item.status==='completed'))setReviewDay(task.day_number);if(!checked&&reviewDay===task.day_number)setReviewDay(null);await client.put(`${API}/tasks/${task.id}`,{...task,status:checked?'completed':'pending'});await Promise.all([load(task.day_number),loadStats()])}
  async function saveCheckin(){const day=reviewDay||reviewDraftDay.current||overview.next_task?.day_number;await client.put(`${API}/checkins/${overview.today}`,{...checkin,learning_day:day});message.success(`Day ${day || ''} 复盘已保存，并同步到学习笔记`);setReviewDay(null);reviewDraftDay.current=null;await Promise.all([load(),loadStats(),loadNotes()])}
  async function completeToday(){const day=reviewDraftDay.current||overview.next_task?.day_number;if(!day)return;setFinishingDay(true);try{const result=await client.post(`${API}/days/${day}/complete`,{...checkin,learning_day:day});message.success(result.message);setReviewDay(null);reviewDraftDay.current=null;setCheckin({actual_minutes:0,gains:'',blockers:'',tomorrow_focus:''});await Promise.all([load(),loadStats(),loadNotes()])}catch(error){message.error(error.message)}finally{setFinishingDay(false)}}
  async function changeTimer(action){const day=overview.next_task?.day_number;if(!day)return;try{setStudyTimer(await client.post(`${API}/timer/${day}/${action}`,{}))}catch(error){message.error(error.message)}}
  ...
  function confirmDeletePlan(plan){Modal.confirm({title:`删除历史计划「${plan.title}」？`,width:520,okText:'确认删除',okButtonProps:{danger:true},cancelText:'取消',content:<Alert type="warning" showIcon title="该计划的任务和完成进度会永久删除；学习笔记与每日打卡记录会保留。"/>,onOk:async()=>{const result=await client.delete(`${API}/plans/${plan.id}`);message.success(result.message);await Promise.all([load(),loadStats()]);}})}
  const phases=useMemo(()=>Object.entries([...tasks].sort((a,b)=>planOrder==='desc'?String(b.planned_date).localeCompare(String(a.planned_date)):String(a.planned_date).localeCompare(String(b.planned_date))).reduce((a,t)=>{(a[`${t.planned_date} · 第 ${t.day_number} 天 · ${t.phase}`]??=[]).push(t);return a},{})),[tasks,planOrder]);
  if(!overview)return <Card loading/>;
  const tabs=[
    {key:'restart',label:'🔄 重新开始',children:<><Card className="section-card" title="从第 1 天重新开始"><Paragraph>创建一份从今天开始的 40 天零基础文档课程。旧计划会进入历史记录，原来的任务、打卡和学习笔记都不会删除。</Paragraph><Space wrap><Tag color="purple">当前：{overview.plan.title}</Tag><Tag>已保存计划 {plans.length} 期</Tag><Button danger type="primary" onClick={confirmRestart}>重新开始学习</Button></Space></Card><Card className="section-card" title="历史学习计划"><List dataSource={plans} renderItem={plan=><List.Item actions={plan.status==='archived'?[<Button key="delete" danger size="small" icon={<DeleteOutlined/>} onClick={()=>confirmDeletePlan(plan)}>删除</Button>]:[]}><List.Item.Meta title={<Space><b>{plan.title}</b><Tag color={plan.status==='active'?'green':'default'}>{plan.status==='active'?'学习中':'已归档'}</Tag></Space>} description={`${plan.original_start_date} — ${plan.projected_end_date}`}/></List.Item>}/></Card></>},
    {key:'today',label:'🌱 今日学习',children:<div className="learning-today"><Row gutter={[16,16]}>
      <Col xs={24} lg={16}><Card className="hero-card"><Text>现在最重要的一步</Text><Title level={2}>{overview.next_task?.title||'40 天计划已完成 🎉'}</Title><Paragraph>{overview.next_task?.details}</Paragraph><Progress percent={overview.progress}/></Card>{overview.next_task&&<DailyExecutionGuide key={overview.next_task.id} task={overview.next_task}/>}<Card title={`今日任务 · ${overview.today}`} className="section-card"><List dataSource={overview.today_tasks} locale={{emptyText:'今天没有任务'}} renderItem={t=><List.Item><Checkbox checked={t.status==='completed'} onChange={e=>toggle(t,e.target.checked)}><b>{t.title}</b><div><Text type="secondary">{t.details} · {t.expected_minutes} 分钟</Text></div><div><Tag>{t.category}</Tag>验收：{t.acceptance_criteria}</div></Checkbox></List.Item>}/></Card></Col>
      <Col xs={24} lg={8}>
        <Card className="study-timer-card" title="⏱️ 今日学习计时">
          <div className="study-timer-value">{String(Math.floor(studyTimer.elapsed_seconds/3600)).padStart(2,'0')}:{String(Math.floor(studyTimer.elapsed_seconds%3600/60)).padStart(2,'0')}:{String(studyTimer.elapsed_seconds%60).padStart(2,'0')}</div>
          <Tag color={studyTimer.status==='running'?'green':studyTimer.status==='paused'?'orange':'default'}>{studyTimer.status==='running'?'计时中':studyTimer.status==='paused'?'已暂停':studyTimer.status==='stopped'?'已结束':'未开始'}</Tag>
          <Space className="study-timer-actions"><Button type="primary" disabled={studyTimer.status==='running'} onClick={()=>changeTimer('start')}>开始</Button><Button disabled={studyTimer.status!=='running'} onClick={()=>changeTimer('pause')}>暂停</Button><Button danger disabled={!['running','paused'].includes(studyTimer.status)} onClick={()=>changeTimer('stop')}>结束</Button></Space>
          <Paragraph type="secondary">进入新一天会自动开始；刷新页面后计时不会丢失。完成今日课程时会自动结束并提交累计时间。</Paragraph>
        </Card>
        <Card title="📝 每日复盘"><Form layout="vertical"><Form.Item label="实际学习分钟（可以手动补充）"><InputNumber min={0} max={1440} value={checkin.actual_minutes} onChange={v=>setCheckin({...checkin,actual_minutes:v||0})}/></Form.Item>{[['gains','今日收获'],['blockers','遇到的问题'],['tomorrow_focus','明日重点']].map(([k,l])=><Form.Item label={l} key={k}><Input.TextArea rows={3} value={checkin[k]} onChange={e=>setCheckin({...checkin,[k]:e.target.value})}/></Form.Item>)}<Alert type="info" showIcon message="完成后会一次提交" description="自动结束计时、完成当天全部任务，保存学习时长和每日复盘，并同步生成 Day 学习笔记。" style={{marginBottom:16}}/><Button type="primary" size="large" block loading={finishingDay} disabled={!overview.next_task} onClick={completeToday}>✅ 完成今天并提交全部内容</Button><Button type="link" block onClick={saveCheckin}>只保存复盘，暂不完成今天</Button></Form></Card>
      </Col>
    </Row></div>},
    {key:'plan',label:'🗺️ 学习计划',children:<><Space className="plan-toolbar"><Tag color="blue">原计划 {overview.plan.original_start_date} — {overview.plan.original_end_date}</Tag><Tag color={overview.deadline_risk?'red':'green'}>预计结束 {overview.plan.projected_end_date}</Tag><Button onClick={()=>setPlanOrder(value=>value==='desc'?'asc':'desc')}>{planOrder==='desc'?'↓ 倒序：最新在前':'↑ 正序：第 1 天在前'}</Button><Button icon={<PlusOutlined/>} onClick={()=>{setTaskModal({});form.resetFields()}}>新增任务</Button></Space><div className="plan-list">{phases.map(([label,rows])=><Card size="small" title={label} key={label} extra={rows[0]?.day_checkin&&<Text type="secondary">已复盘 {rows[0].day_checkin.actual_minutes} 分钟 · {rows[0].day_checkin.gains || '暂无收获记录'}</Text>}>{rows.map(t=><div className="plan-task" key={t.id}><Checkbox checked={t.status==='completed'} onClick={e=>e.stopPropagation()} onChange={e=>toggle(t,e.target.checked)}/><span>{t.category==='复盘'?'✍️':'🧪'} {t.title}</span><Text type="secondary">{t.expected_minutes}m</Text><Button size="small" type="primary" ghost onClick={()=>setReviewTask(t)}>学习 / 复习</Button><Button size="small" onClick={()=>{setTaskModal(t);form.setFieldsValue({...t,planned_date:dayjs(t.planned_date)})}}>编辑</Button></div>)}</Card>)}</div><Modal open={!!reviewTask} title={reviewTask?`第 ${reviewTask.day_number} 天 · ${reviewTask.title}`:''} footer={null} width={1100} onCancel={()=>setReviewTask(null)} destroyOnHidden><DailyExecutionGuide key={reviewTask?.id} task={reviewTask||{day_number:1}}/></Modal></>},
