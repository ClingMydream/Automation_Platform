...
export function LearningPanel({client}) {
  ...
  async function saveCheckin(){await client.put(`${API}/checkins/${overview.today}`,{...checkin,learning_day:overview.next_task?.day_number});message.success(`Day ${overview.next_task?.day_number || ''} 复盘已保存，并同步到学习笔记`);await Promise.all([load(),loadStats(),loadNotes()])}
