...
function AnswerReveal({ children, label = '点击查看参考答案' }) {
  return <Collapse className="answer-reveal" size="small" items={[{ key: 'answer', label, children }]} />;
}

function ReferenceCode({ code }) {
  return <div className="reference-code"><Text strong>参考代码 / 命令</Text><pre className="mastery-code"><code>{code}</code></pre><Text type="secondary">请先理解输入、处理和输出，再按题目要求修改；不要直接复制后跳过运行验证。</Text></div>;
}
...
export function MasteryLearningPanel({ client, isAdmin = false }) {
  ...
  const stepBody = [
    ...
    <div><Alert type="warning" showIcon title="改写不是重新复制" description={content.rewrite_task} /><AnswerReveal label="点击查看改写参考答案"><Paragraph>{content.rewrite_reference_answer}</Paragraph><ReferenceCode code={content.rewrite_reference_code} /></AnswerReveal><Input.TextArea className="code-input" rows={14} value={evidence.modified_code} onChange={(event) => changeEvidence({ modified_code: event.target.value })} placeholder="粘贴你亲自修改后的代码或命令，并在注释中说明改了什么" /></div>,
    <div><Title level={4}>不看答案，自己完成</Title><Paragraph className="mastery-lead">{content.exercise.prompt}</Paragraph><Paragraph type="secondary">提示：{content.exercise.hint}</Paragraph><AnswerReveal><Paragraph>{content.exercise.reference_answer}</Paragraph><ReferenceCode code={content.exercise.reference_code} /></AnswerReveal><Input.TextArea rows={12} value={evidence.exercise_answer} onChange={(event) => changeEvidence({ exercise_answer: event.target.value })} placeholder="写下代码、命令、排查过程或答案" /></div>,
    <div><Title level={4}>确认你知道为什么</Title>{quiz.map((row, index) => <Card size="small" key={row.question} title={`${index + 1}. ${row.question}`} className="quiz-card"><Radio.Group value={evidence.quiz_answers[index]} onChange={(event) => { const answers = [...evidence.quiz_answers]; answers[index] = event.target.value; changeEvidence({ quiz_answers: answers }); }}><Space direction="vertical">{row.options.map((option, optionIndex) => <Radio value={optionIndex} key={option}>{option}</Radio>)}</Space></Radio.Group><AnswerReveal><Paragraph><Text strong>正确答案：{row.reference_answer}</Text></Paragraph><Paragraph>{row.reference_explanation}</Paragraph><ReferenceCode code={content.annotated_code} /></AnswerReveal>{row.is_correct === false && <Alert style={{ marginTop: 12 }} type="warning" title="这次答案还不正确" description={row.explanation} />}{row.is_correct === true && <Alert style={{ marginTop: 12 }} type="success" title="理解正确" description={row.explanation} />}</Card>)}<Title level={5}>用自己的话解释本关</Title><AnswerReveal label="点击查看解释示例"><Paragraph>{content.explanation_reference_answer}</Paragraph><ReferenceCode code={content.annotated_code} /></AnswerReveal><Input.TextArea rows={6} value={evidence.explanation} onChange={(event) => changeEvidence({ explanation: event.target.value })} placeholder="至少 20 个字：为什么要这样写？输入、处理、输出分别是什么？" showCount maxLength={10000} /></div>,
